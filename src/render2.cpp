//---------------------------------------
// Program: render2.cpp
// Purpose: To render convex polygons using
...
void display()
{
   ...
   for (int i=0; i<poly_count; i++)
   {
      ...
      while (!convex)
      {
         ...
         convex = convex_polygon(pt, pt_count);
      }

      // Render convex polygon
      render_polygon(pt, pt_count);
   }
...
{
   if ((key == '+') && (pt_count < MAX_COUNT))
      pt_count++;
   else if ((key == '-') && (pt_count > MIN_COUNT))
      pt_count--;
   glutPostRedisplay();
}
